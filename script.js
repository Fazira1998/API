...
async function getWeather(city) {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=3f9d3ca529c6645c0d5427b85c4c4b54&units=metric`
  ).catch(handleError);
  ...
  async function getTime(data) {
    try {
      const response = await fetch(
        `https://secure.geonames.org/timezoneJSON?lat=${data.coord.lat}&lng=${data.coord.lon}&username=Fazira`
      );
      const responseJson = await response.json();
